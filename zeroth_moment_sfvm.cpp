...
int main()
{
    ...
    vector<double> dtT;
    vector<double> z_m;           // first moment
    for (int n = 0; n < 100; n++) // n is taken as time in equation
    {
        double zm = 0;
        for (int i = 1; i < class_size; i++)
        {
            zm += f_time[n][i] * (x[i] - x[i - 1]);
        }
        z_m.push_back(zm*(pow(1.2, (n+1)*-1)));
            ...
            for (int j = 1; j < class_size; j++)
            {
                for (int k = 1; k < class_size; k++)
                {
                    if (x[j] + x[k] < x[class_size - 1])
                        birth_term += beta[j][k] * f_time[n][j] * f_time[n][k] * (x[j] - x[j - 1]) * (x[k] - x[k - 1]) * weight_const_b(x, i, j, k);
                }
            }
        }
        double death_term = 0.0;
        for (int i = 1; i < class_size; i++) // i is cell no
        {
            for (int j = 1; j < class_size; j++) // upto capital I
            {
                death_term += beta[i][j] * f_time[n][i] * f_time[n][j] * (x[i] - x[i - 1]) * (x[j] - x[j - 1]) * weight_const_d(x, class_size - 1, i, j);
            }
        }
    ...
    cout << "µ(zeroth moment):\n";
    for (int i = 1; i < z_m.size(); i++)
    {
        cout << "µ[n+1], µ[n]+dtT at timestamp " << i << ": " << z_m[i] << " " << z_m[i - 1] + dtT[i - 1] << "\n";
    }
    cout << endl;

    //writing in file
    ofstream zerothMomentData;
    zerothMomentData.open("zeroth_moment_data_sfvm.txt");
    for(int i=0;i<z_m.size();i++){
        zerothMomentData<<i*0.1<<" "<<z_m[i]/z_m[0]<<"\n";
    }
    zerothMomentData.close();
    return 0;
}
