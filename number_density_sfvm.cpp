...
int main()
{
    ...
    {
        cout << "timestamp --> f : "<<i<<" --> "<<accumulate(f_time[i].begin(), f_time[i].end(), 0.0)<<"\n";
    }
    cout << endl;

    //writing in file
    ofstream numberDensityData;
    numberDensityData.open("number_density_data_sfvm.txt");
    for (int i = 0; i < f_time.size(); i++)
    {
        numberDensityData<<i<<" "<<accumulate(f_time[i].begin(), f_time[i].end(), 0.0)*(pow(1.1, -1.12*(i+1)))/accumulate(f_time[0].begin(), f_time[0].end(), 0.0)<<"\n";
    }
