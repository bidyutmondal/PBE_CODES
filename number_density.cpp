...
int main()
{
    ...
    // f = f + del(t)*pbe
    int t_final = 10;
    for (double n = 0; n < t_final; n += 0.1) // n is taken as time in equation
    {
        for (int i = 1; i < class_size; i++) // i is cell no
        {
            ...
            for (int j = 1; j < i; j++)
            {
                ...
                {
                    if (x[j] + x[k] < x[i])
                        birth_term += beta[j][k] * f[j] * f[k] * (((x[j] - x[j - 1]) * (x[k] - x[k - 1])) / (x[i] - x[i - 1])) * weight_const_b(x, i, j, k);
                }
            }
            double death_term = 0.0;
            for (int j = 1; j < class_size; j++) // upto capital I doubt
            {
                death_term += beta[i][j] * f[i] * f[j] * (x[j] - x[j - 1]) * weight_const_d(x, class_size, i, j); // class size var can be changed to something
            }
            // f = f + del(t)(1.0/2*birth_term-death_term)
            f[i] = f[i] + 0.1 * (0.5 * birth_term - death_term); // 0.5 to ommit symmetry
        }
        f_time.push_back(f);
    }
    ...
    //writing in file
    ofstream numberDensityData;
    numberDensityData.open("number_density_data.txt");
    for (int i = 0; i < f_time[99].size()-6; i++)
    {
        numberDensityData<<x[i]<<" "<<f_time[99][i]<<"\n";
    }
    numberDensityData.close();
    return 0;
}
