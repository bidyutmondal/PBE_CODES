...
int main()
{
    ...
    for (int n = 0; n < 100; n++) // n is taken as time in equation
    {
        ...
        for (int i = 1; i < class_size; i++) // i is cell no
        {
            for (int j = 1; j < class_size; j++)
            {
                ...
                {
                    if (x[j] + x[k] < x[class_size - 15])
                        birth_term += beta[j][k] * f_time[n][j] * f_time[n][k] * (x[j] - x[j - 1]) * (x[k] - x[k - 1]) * x[i];
                }
            }
        }
        double death_term = 0.0;
        for (int i = 1; i < class_size; i++) // i is cell no
        {
            for (int j = 1; j < class_size; j++) // upto capital I
            {
                death_term += beta[i][j] * f_time[n][i] * f_time[n][j] * x[i] * (x[i] - x[i - 1]) * (x[j] - x[j - 1]);
            }
        }
    ...
    firstMomentData.open("first_moment_data_sfvm.txt");
    for(int i=1;i<f_m.size();i++){
