...
int main()
{
    ...
    numberDensityData.open("number_density_data_nfvm.txt");
    for (int i = 0; i < f_time.size(); i++)
    {
        numberDensityData<<i<<" "<<accumulate(f_time[i].begin(), f_time[i].end(), 0.0)*(pow(1.1, -1.05*(i+1)))/accumulate(f_time[0].begin(), f_time[0].end(), 0.0)<<"\n";
    }
